...
resource "aws_launch_template" "backend_launch_template" {
  name_prefix            = "backend-launch-template" # Prefix for the launch template name
  image_id               = var.image_id                   # Use the dynamically fetched Amazon Linux 2 AMI ID
  vpc_security_group_ids = [var.backend_sg_id]       # Security group for instances launched from this template
  instance_type          = var.instance_type              # Instance type for the backend app (can be changed as needed)
...
resource "aws_autoscaling_group" "backend_asg" {
  ...
  launch_template {
    id      = aws_launch_template.backend_launch_template.id # Reference the launch template ID
